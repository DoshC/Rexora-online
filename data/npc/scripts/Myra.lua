...
local function creatureSayCallback(cid, type, msg)
	...
	local storage = OutfitQuest.MageSummonerTiaraCloakAddon
	...
	elseif msgcontains(msg, "tiara") then
		if getPlayerStorageValue(cid,storage) < 1 then
			...
			TopicState[cid] = 1
		elseif getPlayerStorageValue(cid,storage) >= 1 and getPlayerStorageValue(cid,storage) < 10 then
	...
	elseif config[msg:lower()] then
		if getPlayerStorageValue(cid,storage) == config[msg:lower()]["value"] then
	...
	elseif msgcontains(msg, "yes") then
		if TopicState[cid] == 1 then
			npcHandler:say("Okay, great! You see, I need a few magical ingredients which I've run out of. First of all, please bring me 70 bat wings. ...",cid)
			addEvent(selfSay, 3000,	"Then, I urgently need a lot of red cloth. I think 20 pieces should suffice. ...",cid)
			addEvent(selfSay, 6000,	"Oh, and also, I could use a whole load of ape fur. Please bring me 40 pieces. ...",cid)
			addEvent(selfSay, 9000,	"After that, um, let me think... I'd like to have some holy orchids. Or no, many holy orchids, to be safe. Like 35. ...",cid)
			addEvent(selfSay, 12000,	"Then, 10 spools of spider silk yarn, 60 lizard scales and 40 red dragon scales. ...",cid)
			addEvent(selfSay, 15000,	"I know I'm forgetting something.. wait... ah yes, 15 ounces of magic sulphur and 30 ounces of vampire dust. ...",cid)
			addEvent(selfSay, 18000,	"That's it already! Easy task, isn't it? I'm sure you could get all of that within a short time. ...",cid)
			addEvent(selfSay, 21000,	"Did you understand everything I told you and are willing to handle this task?", cid)
			TopicState[cid] = 2
		...
		elseif TopicState[cid] == 3 then
			if getPlayerItemCount(cid,itemId) >= amount then
				...
				setPlayerStorageValue(cid,storage, math.max((getPlayerStorageValue(cid,storage) or 0)) + 1)
