...
local eclipse = {x = 32659, y = 31915, z = 0}
local farmine =	{x = 32983, y = 31339, z = 1}
...
function onThink()
	if(rnd_sounds < os.time()) then
		...
		if(math.random(100) < 25) then
			npcHandler:say("Feel the wind in your hair during one of my carpet rides!", TALKTYPE_SAY)
		end
	end
	npcHandler:onThink()
end
...
local pid = NPCHANDLER_CONVBEHAVIOR == CONVERSATION_DEFAULT and 0 or cid
	...
	local travelNode = keywordHandler:addKeyword({'edron'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'Do you seek a ride to Edron for 60 gold?'})
		travelNode:addChildKeyword({'yes'}, StdModule.travel, {npcHandler = npcHandler, premium = true, level = 0, cost = 60, destination = {x = 33175, y = 31764, z = 6} })
		travelNode:addChildKeyword({'no'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, reset = true, text = 'You shouldn\'t miss the experience.'})
...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "eclipse") then
		if getPlayerStorageValue(cid, TheInquisition.Questline) == 4 then
	...
	elseif msgcontains(msg, "farmine") then
		if getPlayerStorageValue(Storage.TheNewFrontier.Mission10) == 1 then
	...
	elseif msgcontains(msg, "yes") then
		if TopicState[cid] == 1 then	
			...
			doTeleportThing(cid,eclipse)
			doSendMagicEffect(getPlayerPosition(cid),CONST_ME_TELEPORT)
		elseif TopicState[cid] == 2 then
			if doPlayerRemoveMoney(60) then
				doSendMagicEffect(getPlayerPosition(cid),CONST_ME_TELEPORT)
