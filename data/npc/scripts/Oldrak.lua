...
local function creatureSayCallback(cid, type, msg)
	...
	if (msgcontains(msg, 'mission') or msgcontains(msg, 'demon oak')) then
		if(getPlayerStorageValue(cid,DemonOak.Done) < 1 and getPlayerStorageValue(cid,DemonOak.Progress) < 2) then
			npcHandler:say("How do you know? Did you go into the infested area?", cid)
			TopicState[cid] = 1
		elseif (getPlayerStorageValue(cid,DemonOak.Progress) == 2 and getPlayerStorageValue(cid,DemonOak.Done) < 1) then
			npcHandler:say("You better don't return here until you've defeated the Demon Oak.", cid)
		elseif (getPlayerStorageValue(cid,DemonOak.Done) == 1) then
			npcHandler:say(cid,"You chopped down the demon oak?!? Unbelievable!! Let's hope it doesn't come back. As long as evil is still existent in the soil of the plains, it won't be over. Still, the demons suffered a setback, that's for sure. ...", cid)
			addEvent(selfSay, 3500,"For your brave action, I tell you a secret which has been kept for many many years. There is an old house south of the location where you found the demon oak. There should be a grave with the name 'Yesim Adeit' somewhere close by. ...", cid)
			addEvent(selfSay, 7000,"It belongs to a Daramian nobleman named 'Teme Saiyid'. I knew him well and he told me -almost augured- that someone will come who is worthy to obtain his treasure. I'm sure this 'someone' is you. Good luck in finding it!", cid)
			setPlayerStorageValue(cid,DemonOak.Done, 2)
		end
	elseif (msgcontains(msg, 'yes') and TopicState[cid] == 1) then
		if (getPlayerStorageValue(cid,DemonOak.Progress) == 1) then
			...
			setPlayerStorageValue(cid,DemonOak.Progress, 2)
			TopicState[cid] = 0
		else
	...
	elseif (msgcontains(msg, 'axe') or msgcontains(msg, 'hallowed axe')) then
		if (getPlayerStorageValue(cid,DemonOak.Progress) == 2) then
	...
	elseif (msgcontains(msg, 'yes') and TopicState[cid] == 2) then
		if (getPlayerStorageValue(cid,DemonOak.Progress) == 2) then
	...
	elseif (msgcontains(msg, 'no') and TopicState[cid] == 1) then
		...
		TopicState[cid] = 0
	elseif (msgcontains(msg, 'no') and TopicState[cid] == 2) then
