...
function onThink()
	if lastSound < os.time() then
		...
		if math.random(100) < 25 then
			npcHandler:say("Gems and jewellery! Best prices in town!", TALKTYPE_SAY)
		end
	end
	npcHandler:onThink()
end
...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "addon") or msgcontains(msg, "outfit") or msgcontains(msg, "hat") then
		local addonProgress = getPlayerStorageValue(cid,OutfitQuest.Citizen.AddonHat)
	...
	if TopicState[cid] == 1 then
		if msgcontains(msg, 'yes') then
			...
			setPlayerStorageValue(cid,OutfitQuest.Citizen.AddonHat, 1)
			setPlayerStorageValue(cid,OutfitQuest.Citizen.MissionHat, 1)
	...
	elseif TopicState[cid] == 2 then
		if msgcontains(msg, 'yes') then
			...
			elseif getPlayerItemCount(cid,5902) < 50 then
				...
				setPlayerStorageValue(cid,OutfitQuest.Citizen.MissionHat, 0)
				setPlayerStorageValue(cid,OutfitQuest.Citizen.AddonHat, 2)
