...
local function greetCallback(cid)
	if not getCreatureCondition(cid,CONDITION_DRUNK) then
...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, 'potion') then
		if getPlayerStorageValue(cid,OutfitQuest.AssassinBaseOutfit) < 1 then
	...
	elseif config[msg] and TopicState[cid] == 0 then
		if getPlayerStorageValue(cid,OutfitQuest.AssassinBaseOutfit) == config[msg].storageValue then
	...
	elseif msgcontains(msg, 'secret') then
		if getPlayerStorageValue(cid,OutfitQuest.AssassinBaseOutfit) == 8 then
			...
			setPlayerStorageValue(cid,OutfitQuest.AssassinBaseOutfit, 9)
		end
	elseif msgcontains(msg, 'yes') then
		if TopicState[cid] == 1 then
			npcHandler:say('You\'re a true buddy. I promise I will t-try to avoid killing you even if someone asks me to. <hicks> ...',cid)
			addEvent(selfSay, 3000,	'Listen, I have this old formula from my grandma. <hicks> It says... 30 {bonelord eyes}... 10 {red dragon scales}. ...',cid)
			addEvent(selfSay, 6000,	'Then 30 {lizard scales}... 20 {fish fins} - ew, this sounds disgusting, I wonder if this is really a potion or rather a cleaning agent. ...',cid)
			addEvent(selfSay, 9000,	'Add 20 ounces of {vampire dust}, 10 ounces of {demon dust} and mix well with one flask of {warrior\'s sweat}. <hicks> ...',cid)
			addEvent(selfSay, 12000,	'Okayyy, this is a lot... we\'ll take this step by step. <hicks> Will you help me gathering 30 {bonelord eyes}?', cid)
			TopicState[cid] = 2
		elseif TopicState[cid] == 2 then
			...
			setPlayerStorageValue(cid,OutfitQuest.AssassinBaseOutfit, 1)
		...
		elseif TopicState[cid] == 3 then
			...
			setPlayerStorageValue(cid,OutfitQuest.AssassinBaseOutfit, getPlayerStorageValue(cid,OutfitQuest.AssassinBaseOutfit) + 1)
