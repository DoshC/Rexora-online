...
function onUse(cid, item, fromPosition, itemEx, toPosition)
	-- Eclipse
	if(itemEx.actionid == 14026) then
		doRemoveItem(item.uid, 1)
		doSendMagicEffect(toPosition, CONST_ME_FIREAREA)
		setPlayerStorageValue(cid,TheInquisition.Questline, 5)
		setPlayerStorageValue(cid,TheInquisition.Mission02, 2) -- The Inquisition Questlog- "Mission 2: Eclipse"
	end
	-- Haunted Ruin
	if(itemEx.actionid == 14028) then
		if(getPlayerStorageValue(cid,TheInquisition.Questline) == 12) then
			doSummonCreature("Pirate Ghost", toPosition)
			doRemoveItem(item.uid, 1)
			setPlayerStorageValue(cid,TheInquisition.Questline, 13)
			setPlayerStorageValue(cid,TheInquisition.Mission04, 2) -- The Inquisition Questlog- "Mission 4: The Haunted Ruin"
	...
	elseif(itemEx.itemid == 8759) then
		...
		if(getPlayerStorageValue(cid,TheInquisition.Questline) < 22) then
			setPlayerStorageValue(cid,TheInquisition.Questline, 22)
			setPlayerStorageValue(cid,TheInquisition.Mission07, 2) -- The Inquisition Questlog- "Mission 7: The Shadow Nexus"
		end
